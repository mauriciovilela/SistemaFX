...
public class VendedorBLL implements Serializable {
	...
	public TbVendedor porId(Integer id) {
		return manager.find(TbVendedor.class, id);
	}

	@SuppressWarnings("unchecked")
	public List<TbVendedor> listar() {
		Session session = manager.unwrap(Session.class);
		Criteria criteria = session.createCriteria(TbVendedor.class);
		return criteria.list();
	}

	@SuppressWarnings("unchecked")
	public List<TbVendedor> filtrados(FiltroIN filtro) {
	...
	public TbVendedor guardar(TbVendedor item) {
		return manager.merge(item);
	}
	
	@Transactional
	public void remover(TbVendedor item) {
		try {
