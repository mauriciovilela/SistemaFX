...
public class LinhaBLL implements Serializable {
	...
	public TbLinha porId(Integer id) {
		return manager.find(TbLinha.class, id);
	}

	@SuppressWarnings("unchecked")
	public List<TbLinha> listar() {
		Session session = manager.unwrap(Session.class);
		Criteria criteria = session.createCriteria(TbLinha.class);
		criteria.add(Restrictions.eq("tbFilial.id", SessionContext.getInstance().getCdFilial())); // Filtro da Filial
		return criteria.list();
	}

	@SuppressWarnings("unchecked")
	public List<TbLinha> filtrados(FiltroIN filtro) {
	...
	public TbLinha guardar(TbLinha item) {
		return manager.merge(item);
	}
	
	@Transactional
	public void remover(TbLinha item) {
		try {
